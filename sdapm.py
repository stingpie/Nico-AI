        ...
        if correct!=None:
                ...
                for i in range(loops):
                    ...
                    if(self.autoscale): ## rescale to size of correct output
                        ...
                        result = scale * F.normalize(result - shift, dim=-1, p=float('inf'))#result * (T.linalg.vector_norm(correct[:,i], ord=1)/T.linalg.vector_norm(result, ord=1))
            ...
            for i in range(loops):
                ...
                if(self.autoscale): ## rescale to size of input
                    ...
                    result = scale* F.normalize(result-shift, dim=-1, p=float('inf'))#result * (T.linalg.vector_norm(x[:,i], ord=1)/T.linalg.vector_norm(result, ord=1))
                results[:,i]=result
...
class SDAPM(T.nn.Module):
    ...
    def __init__(self, data_width, max_steps=10, mem_len=100, layer_gain=None, var_lr=False, param_mem_mats=False, sparse_dat='none', sparse_router='top 3', router_search = False, device = 'cpu'):
    ...
    def engram_info(self):
        ...
        atten_dat_size= self.data_width*9+1 + 2*int(math.ceil(math.sqrt(self.mem_len)))
        zero_padding = T.zeros((self.memories[self.steps_since_reset].batch_size[0], self.mem_len, math.ceil(atten_dat_size/self.num_attn_heads)*self.num_attn_heads - atten_dat_size), device=self.device)
    ...
    def reset(self, x):
        self.memories=[tensordict.TensorDict({
                'm':self.initial_mem_mats.unsqueeze(0).expand(x.size(0),-1,-1,-1).clone(),
                'bv':self.initial_bv.unsqueeze(0).expand(x.size(0),-1,-1).clone(),
                'bw':self.initial_bw.unsqueeze(0).expand(x.size(0),-1,-1).clone(),
                'w':self.initial_worth.unsqueeze(0).expand(x.size(0),-1,-1).clone(),
    ...
    def deNaNed(self, x):
        if(isinstance(x, tensordict.TensorDict)):
            for key in x.keys():
    ...
    def forward(self,x, correct=None, reset=True):
        ...
        if correct!=None:
                ...
                est_worth, self.hx_cx = self.worth_model(T.cat((x,correct), dim=-1), self.hx_cx)
                estimated_worth = T.sum(est_worth, dim=-1)
                loss_estimated_worth = F.mse_loss(T.log(inaccuracies+1e-6), estimated_worth)
    ...
    def read(self, x, correct=None):
        ...
        if(correct!=None):
            ...
            worth_est, self.hx_cx = self.worth_model(T.cat((x,result[-1]), dim=-1).unsqueeze(-2).detach(), self.hx_cx)
            worth_est = T.sum(worth_est,dim=-1)
            d_worth =T.sum(T.stack(routing,dim=1), dim=1)*worth_est.unsqueeze(-2).expand(-1,self.mem_len,-1)/self.max_steps
    ...
    def memorize(self, x, y, log_inaccuracy=None): ## overly simplistic. Memorry consolidation is done elsewhere.
        ...
        memorize_mem_vals ={
                ...
                'bw':new_bw.unsqueeze(-2),
                'w':-log_inaccuracy.unsqueeze(-2) if log_inaccuracy!=None else T.median(self.memories[self.steps_since_reset]['w'].squeeze(-1),dim=-1)[0].unsqueeze(-1).unsqueeze(-2),
                't':pos_encoding(int(math.ceil(math.sqrt(self.mem_len))), self.steps_since_reset).to(self.device).unsqueeze(-2).expand(x.size(0),-1,-1)
                }


        mask = T.zeros((x.size(0), self.mem_len,1), device=self.device)
        mask[idx, min_worth_idx] = 1
    ...
    def consolidate(self, num_k=5, loops=1):
        ...
        consolidate_mem_vals = self.memories[self.steps_since_reset].clone().detach()
        consolidate_mem_mask = {
            'm':T.zeros((self.memories[self.steps_since_reset].batch_size[0], self.mem_len,1,1), device=self.device),
            'bv':T.zeros((self.memories[self.steps_since_reset].batch_size[0], self.mem_len,1), device=self.device),
            'bw':T.zeros((self.memories[self.steps_since_reset].batch_size[0], self.mem_len,1), device=self.device),
            'w':0,
            't':T.zeros((self.memories[self.steps_since_reset].batch_size[0], self.mem_len,1), device=self.device)
            }
        ...
        for i in range(loops):
            ...
            consolidate_mem_vals['bv'][idx,most_similarities] = new_bv
            consolidate_mem_vals['bw'][idx,most_similarities] = new_bw
            consolidate_mem_vals['t'][idx,most_similarities] = pos_encoding(int(math.ceil(math.sqrt(self.mem_len))), self.steps_since_reset).to(self.device)
